...
require 'voxpupuli/test/rake/symlinks'

namespace :check do
  desc 'Fails if symlinks are present in directory'
  task :symlinks do
    symlinks = Voxpupuli::Test::Rake::Symlinks.new.check
...
task check: ['check:symlinks', 'check:test_file', 'check:dot_underscore', 'check:git_ignore', 'check:trailing_whitespace']
